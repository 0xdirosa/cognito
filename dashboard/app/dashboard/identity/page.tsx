...
export default function AgentIdentity() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">Agent Identity</h1>

      {/* Metadata Card */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
        <h2 className="text-sm font-semibold text-slate-300 mb-4">Metadata</h2>
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-white">{agent.network}</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Chain ID</p>
            <p className="font-mono text-white">{agent.chainId}</p>
          </div>
        </div>

        {/* Capabilities */}
        <div className="mt-4 pt-4 border-t border-slate-800">
          <p className="text-slate-500 text-sm mb-2">Capabilities</p>
      ...
      {/* IPFS Metadata */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
        <h2 className="text-sm font-semibold text-slate-300 mb-3">IPFS Metadata</h2>
        <div className="flex items-center gap-3">
          <code className="font-mono text-sm text-accent bg-slate-800 px-3 py-2 rounded flex-1 truncate">
            {agent.metadataUri}
          </code>
          <button
      ...
      {/* On-Chain Identity */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
        <h2 className="text-sm font-semibold text-slate-300 mb-4">On-Chain Identity</h2>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-slate-500 mb-1">Token ID</p>
            <p className="font-mono text-white">#{identity.tokenId}</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Owner Address</p>
            <p className="font-mono text-white">{formatAddress(identity.owner)}</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Contract Address</p>
            <p className="font-mono text-white">{formatAddress(identity.contractAddress)}</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Explorer</p>
            <a
      ...
      {/* Reputation */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
        <h2 className="text-sm font-semibold text-slate-300 mb-4">Reputation</h2>
        <div className="grid grid-cols-3 gap-4 text-sm">
          ...
          <div>
            <p className="text-slate-500 mb-1">Events on-chain</p>
            <p className="font-mono text-white">{reputation.eventCount}</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Last Tag</p>
            <p className="text-white">{reputation.lastTag ?? "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-slate-300 mb-4">Validation</h2>
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div>
          ...
          <div>
            ...
            <p className="font-mono text-white">{validation.response}/100</p>
          </div>
          <div>
            <p className="text-slate-500 mb-1">Tag</p>
            <p className="text-white">{validation.tag ?? "—"}</p>
          </div>
        </div>
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-300">Agent Network</h2>
          <p className="text-xs text-slate-500 mt-0.5">{agents.length} agents registered on Arc Testnet</p>
        </div>
        {agents.length > 5 && (
          <button
        ...
        {visible.map((a) => (
          <div key={a.agentId} className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-slate-500 text-xs max-w-[120px] truncate">{a.name.slice(0, 30)}</span>
            </div>
            <div className="flex items-center gap-3">
              {a.capabilities.length > 0 ? (
                <span className="text-xs text-slate-400">{a.capabilities.join(", ")}</span>
              ) : (
                <span className="text-xs text-slate-600">—</span>
              )}
              <span className="font-mono text-xs text-slate-500">{formatAddress(a.owner)}</span>
              {a.paid && (
      ...
      <div className="mt-4 pt-4 border-t border-slate-800">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="text-slate-600">→</span>
          <span className="font-mono text-slate-500">{formatAddress("0x58E688E56d048D4C7BA41d66bBC50A64C12B0B66")}</span>
        </div>
        <a
