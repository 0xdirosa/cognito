...
export async function GET(req: NextRequest) {
  try {
    ...
    // Return mock inference history from dashboard inference data
    ...
    const items = INFERENCE_HISTORY.slice(start, start + limit);

    return NextResponse.json({
      success: true,
      data: items,
      pagination: { page, limit, total: INFERENCE_HISTORY.length },
    });
  } catch (err: any) {
...
export async function POST() {
  if (process.env.NEXT_PUBLIC_IS_LOCAL !== "true") {
    return NextResponse.json(
      { success: false, error: "Inference trigger requires local environment with Circle SDK", code: "LOCAL_ONLY" },
      { status: 501 },
    );
  }

  return NextResponse.json(
    { success: false, error: "POST inference trigger not yet implemented in local mode" },
    { status: 501 },
  );
}
